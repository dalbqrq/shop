...
<script>
  ...
  function parcelasDisponiveis() {
	...
	var total = installments[0].installmentAmount;
        ...
        for (var i in installments) {
          ...
          // daniel - dalbqrq@gmail - limita valor minimo de parcelamento em 400 reais
          if ( total < 400 ) {
            break; 
          } 
    ...
    PagSeguroDirectPayment.getBrand({
      ...
      success: function(response) {
        ...
        if (response.brand.expirable) {
          $("#expiraCartao").css('display', 'block'); //none
        } else {
        ...
        if (response.brand.cvvSize > 0) {
          ...
          $("#cvvCartao").css('display', 'block'); //none
        }
  ...
  function showModal() {
      ...
      $("#modal-body").html("Iniciando transação...");
      $("#aguarde").modal("show");
  }
  ...
  function pagarBoleto(senderHash) {
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        $("#modal-body").html("Processando pagamento por boleto...");
        if (!(data.paymentLink)) {
          //alert(data);
          $("#modal-title").html("<font color='red'>Erro - pagarBoleto()</font>");
    ...
    function pagarCartao(senderHash) {
      ...
      PagSeguroDirectPayment.createCardToken({
        ...
        error: function (response) {
          if (response.error) {
            $("#modal-title").html("<font color='red'>Erro - pagarCartao() 1</font>");
      ...
      $.ajax({
        ...
        success: function(data) {
          //console.log(data);
          $("#modal-body").html("Processando pagamento por cartão...");

          if (data.error) {
            if (data.error.code == "53037") {
              $("#creditCardPaymentButton").click();
            } else {
              $("#modal-title").html("<font color='red'>Erro - pagarCartao() 2</font>");
            ...
            $.ajax({
              ...
              success: function(data) {
                $("#modal-body").html("Status pagamento ok...");
				//var status = data.status;
                if (data.status == "7") {
                  //alert(data);
                  $("#modal-title").html("<font color='red'>Erro<pagarCartao() 3/font>");
                  ...
                  window.location = "https://meuestilodecor.com.br/shop/paymentCheckout.php?transaction_id=" + data.code;
                  setTimeout(function () {
                    $("#modal-body").html("");
                    $("#modal-title").html("<font color='green'>Sucesso!</font>")

                    $("#modal-body").html("Caso você não seja redirecionado para a nossa página de instruções, clique no botão abaixo.<br /><br /><a href='https://meuestilodecor.com.br/shop/paymentCheckout.php?transaction_id=" + data.code + "'><center><button class='btn-success btn-block btn-lg'>Ir para a página de instruções</button></center></a>");
                  }, 2000);
                }
